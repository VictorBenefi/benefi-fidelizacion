...
export async function POST(req: Request) {
  try {
    ...
    const usuarioAnulador =
      body?.usuarioAnulador
    ...
    // Buscar el movimiento, pero únicamente
    // dentro del comercio autenticado
    const {
      data: movimiento,
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Movimiento no encontrado o no pertenece a este comercio.",
        },
        { status: 404 }
      );
    }
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "No se puede anular un movimiento reverso.",
        },
        { status: 400 }
      );
    }
    ...
    if (
      movimiento.anulado_por_movimiento_id
    ) {
      return NextResponse.json(
        {
          ok: false,
          error:
            "El movimiento ya tiene una anulación registrada.",
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "No se pudo calcular el saldo actual del cliente.",
        },
        { status: 500 }
      );
    }
    ...
    const nroTicketReverso =
      movimiento.nro_ticket
        ? `${movimiento.nro_ticket}-ANUL`
        : `ANUL-${movimiento.id.slice(
            0,
            8
          )}`;

    // Crear reverso siempre dentro
    ...
    const {
      ...
      .insert({
        ...
        monto_compra:
          movimiento.monto_compra,
        nro_ticket:
          nroTicketReverso,
        observaciones:
          observacionesReverso,
        fecha: fechaAhora,
        promocion_id:
          movimiento.promocion_id,
        estado: "activo",
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "No se pudo crear el movimiento reverso.",
        },
        { status: 500 }
      );
    }

    // Actualizar el original únicamente
    // si pertenece al comercio autenticado
    const { error: updateOriginalError } =
      await supabaseAdmin
        .from("movimientos_puntos")
        .update({
          estado: "anulado",
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "No se pudo marcar el movimiento original como anulado.",
        },
        { status: 500 }
      );
    }
    ...
    console.error(
      "Error en /api/movimientos/anular:",
      error
    );
