...
type OperacionAgrupada = {
  ...
  estado_visual: 'OPERACION' | 'ANULADA' | 'ANULACION'
...
function agruparOperaciones(movimientos: Movimiento[]): OperacionAgrupada[] {
  ...
  for (const [operacionId, items] of mapa.entries()) {
    ...
    const esOperacionAnulacion = items.length > 0 && items.every((m) => m.es_reverso === true)
    const operacionAnulada = items.some((m) => m.estado === 'anulado')

    let estadoVisual: OperacionAgrupada['estado_visual'] = 'OPERACION'
    if (esOperacionAnulacion) {
      estadoVisual = 'ANULACION'
    } else if (operacionAnulada) {
...
export async function POST(req: Request) {
  try {
    ...
    if (!usuario_id) {
      return NextResponse.json({ ok: false, error: 'usuario_id es obligatorio' }, { status: 400 })
    }
    ...
    if (!fecha_desde || !fecha_hasta) {
      return NextResponse.json({ ok: false, error: 'Debés informar fecha_desde y fecha_hasta' }, { status: 400 })
    }
    ...
    if (Number.isNaN(desde.getTime()) || Number.isNaN(hasta.getTime())) {
      return NextResponse.json({ ok: false, error: 'Las fechas son inválidas' }, { status: 400 })
    }
    ...
    const operaciones = agruparOperaciones((data || []) as Movimiento[])
    ...
    const nombreArchivo = `cliente_${usuario_id}_${fecha_desde}_a_${fecha_hasta}.csv`
