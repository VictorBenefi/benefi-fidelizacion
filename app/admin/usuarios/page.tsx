...
const totalPuntos = movimientosActivos
  .filter((m) => m.tipo === 'carga')
  .reduce((acc, m) => acc + Number(m.puntos || 0), 0)

const totalCanjeados = movimientosActivos
...
async function verDetalle(usuario: Usuario) {
  ...
  const usuariosFiltrados = usuarios.filter((u) => {
    const coincideBusqueda =
      `${u.nombre_completo || ""} ${u.dni || ""} ${u.email || ""} ${u.comercio_nombre || ""}`
        .toLowerCase()
    ...
    <>
      <div className="p-8">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          <input
            type="text"
            placeholder="Buscar por nombre, DNI, email o comercio..."
            value={busqueda}
                ...
                <tr key={u.id} className="border-t">
                  ...
                  <td className="p-3">
                    <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">
                      {u.comercio_nombre || "Sin comercio"}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="max-h-[80vh] w-[700px] overflow-auto rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mb-4 grid grid-cols-2 gap-4 text-sm">
              ...
              <div><b>Comercio:</b> {usuarioSeleccionado.comercio_nombre || "Sin comercio"}</div>
                ...
                <tbody>
                  {movimientos.map((m) => (
                    <tr
                      ...
                      className="border-t"
                      style={{
                        opacity: m.estado === "anulado" ? 0.55 : 1,
                        background: m.estado === "anulado" ? "#fef2f2" : "transparent",
                      }}
                    >
                      <td className="p-2">
                        {m.tipo || "-"}
                        {m.estado === "anulado" && (
                          <span style={{ color: "#991b1b", marginLeft: 6, fontWeight: 700 }}>
                            (ANULADO)
                          </span>
                        )}
                      </td>
